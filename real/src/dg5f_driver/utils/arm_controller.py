...
from .robot_utils import Proprio, position_action_to_delta_action
import time 


@dataclass
class ArmControllerConfig:
    server_ip: str = "tcp://172.16.0.12:4242"
    max_pos_delta: float = 0.05
    max_euler_delta: float = 0.2


class ArmController:
    def __init__(self, cfg: ArmControllerConfig):
    ...
    def get_proprio(self) -> Proprio:
        ...
        proprio = Proprio(
            ...
            joint_pos=proprio_dict["joint_pos"],
            joint_vel=proprio_dict["joint_vel"]
        )
        return proprio

    def delta_control(self, delta_pos: np.ndarray, delta_euler: np.ndarray):
        ...
        self.rpc.update(new_pos.tolist(), new_quat.tolist())
        self.curr_proprio = None

    def position_control(self, new_pos: np.ndarray, new_euler: np.ndarray):
    ...
    def joint_control(self, target_qpos: np.ndarray):
        self.rpc.update(target_qpos.tolist())